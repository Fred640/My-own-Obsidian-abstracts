...
```
@register.inclusion_tag('<AppName>/listCategories.hmtl')   #указываем путь к шаблону
def show_categories():
	cats = views.cats_db
	return {"cats":cats}                                 #передаем данные в шаблон
```

перед использование тега в шаблоне его нужно загрузить `{% load <TagName> %}` или `{% load <TagsFileName> %}`
